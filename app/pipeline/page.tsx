...
export function middleware(req: NextRequest) {
  ...
  // Protect pipeline and admin pages
  if (pathname.startsWith('/pipeline') || pathname.startsWith('/admin')) {
...
export default function PipelinePage() {
  ...
  function addLog(stage: string, status: 'ok' | 'error' | 'running', message: string) {
    setLogs(prev => {
      const existing = prev.findIndex(l => l.stage === stage);
      const entry = { stage, status, message };
      if (existing >= 0) {
        const updated = [...prev];
  ...
  async function runPipeline() {
    ...
    try {
      // ── Step 1: Research ─────────────────────────────────
      setStage('research');
      addLog('Research', 'running', 'Finding trending topics...');
      const r1 = await post('/api/agents/research', { niche: topic });
      ...
      const r2 = await post('/api/agents/seo', { topics });
      ...
      const r3 = await post('/api/agents/content', { brief });
      ...
      const r4 = await post('/api/agents/image', { title: article.title, slug: article.slug });
      if (!r4.ok) throw new Error(r4.error);
      const img = r4.results?.[0];
      addLog('Image', 'ok', `Image ready (${img?.source})`);

      // ── Step 5: Publish ──────────────────────────────────
      setStage('publish');
      addLog('Publish', 'running', 'Committing to GitHub...');
      const r5 = await post('/api/agents/publisher', { slug: article.slug });
      if (!r5.ok) throw new Error(r5.errors?.[0]?.error || 'Publish failed');
      const published = r5.published?.[0];
      addLog('Publish', 'ok', 'Live on Vercel!');

      setStage('done');
      setResult({ url: published?.url, title: article.title });
              ...
              return (
                <div key={s} style={{
                  ...
                  <div style={{
                    ...
                    color: log ? statusColor[log.status] : '#444',
                    flexShrink: 0,
                  ...
                  {/* Stage info */}
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: '#e5e5e5' }}>{s} agent</div>
                    {log && (
                      <div style={{ fontSize: 12, color: statusColor[log.status], marginTop: 2 }}>
                        {log.message}
                      </div>
                    )}
                  </div>
        ...
        {stage === 'done' && result && (
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              ...
              <button
                onClick={() => { setStage('idle'); setLogs([]); setTopic(''); setResult(null); }}
                style={{
                  padding: '8px 18px',
